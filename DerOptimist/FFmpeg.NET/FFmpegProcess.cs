...
namespace FFmpeg.NET
{
    internal sealed class FFmpegProcess
    {
        ...
        private async Task ExecuteAsync(ProcessStartInfo startInfo, FFmpegParameters parameters, CancellationToken cancellationToken = default)
        {
            ...
            {
                ffmpegProcess.ErrorDataReceived += (sender, e) => OnData(new ConversionDataEventArgs(e.Data, parameters.InputFile, parameters.OutputFile, parameters.ConversionOptions));
                ffmpegProcess.ErrorDataReceived += (sender, e) => FFmpegProcessOnErrorDataReceived(e, parameters, ref caughtException, messages);

                _ = await ffmpegProcess.WaitForExitAsync((exitCode) => OnException(messages, parameters, exitCode, caughtException), cancellationToken);
                ...
                {
                    OnConversionCompleted(new ConversionCompleteEventArgs(parameters.InputFile, parameters.OutputFile, messages, parameters.ConversionOptions));
                }
            }
        }

        public (string output, string err) ExecuteCustomSync(string arguments, string ffmpegFilePath)
        {
            var startInfo = GenerateStartInfo(ffmpegFilePath, arguments);
            ...
            using (var ffmpegProcess = new Process() { StartInfo = startInfo })
            {
                ...
                string err = ffmpegProcess.StandardError.ReadToEnd();
                //Debug.WriteLine(err);
        ...
        private void OnException(List<string> messages, FFmpegParameters parameters, int exitCode, Exception caughtException)
        {
            ...
            OnConversionError(new ConversionErrorEventArgs(exception, parameters.InputFile, parameters.OutputFile, messages, parameters.ConversionOptions));
        }
        ...
        private void FFmpegProcessOnErrorDataReceived(DataReceivedEventArgs e, FFmpegParameters parameters, ref Exception exception, List<string> messages)
        {
            ...
            try
            {
                ...
                if (RegexEngine.IsProgressData(e.Data, out var progressData))
                {
                    ...
                    OnProgressChanged(new ConversionProgressEventArgs(progressData, parameters.InputFile, parameters.OutputFile, parameters.ConversionOptions));
                }
            }
            catch (Exception ex)
            {
